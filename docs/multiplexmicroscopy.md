...
| Attribute | Type | Required | Pattern | Description |
|-----------|------|----------|---------|-------------|
| `CHANNEL_METADATA_ID` | string | Yes | `^syn\d+$` | Unique identifier specifying the location of the required channel metadata (Synapse ID) |
...
| Attribute | Type | Required | Pattern | Description |
|-----------|------|----------|---------|-------------|
| `FILE_FORMAT` | string | Yes | `^(ome-tiff|ome\.tiff|tiff|tif)$` | Format of the segmentation mask file (should be ome-tiff for Level 3) |
...
| `FILE_FORMAT` | string | Yes | `^(csv|h5ad)$` | Format of the feature table file (csv or h5ad for Level 4) |
