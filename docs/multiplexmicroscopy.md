...
| `IMAGING_ASSAY_TYPE` | See [ImagingAssayType](#imagingassaytype) enum below | Yes | Type of imaging assay |
...
| Attribute | Type | Required | Description |
|-----------|------|----------|-------------|
| `FILE_FORMAT` | string | Yes | Format of the segmentation mask file (should be ome-tiff for Level 3) |
...
| `MATRIX_TYPE` | See [MatrixTypeEnum](#matrixtype) enum below | Yes | Type of feature matrix (raw counts, normalized, etc.) |
