...
| `DISSOCIATION_METHOD` | DissociationMethodEnum<br/>`Enzymatic`, `Mechanical`, `Other`, `Unknown` | Yes | Method used to dissociate tissue into single cells |
| `LIBRARY_CONSTRUCTION_METHOD` | See [LibraryConstructionMethodEnum](#libraryconstructionmethod) enum below | Yes | Method used to construct the sequencing library |
| `NUCLEIC_ACID_SOURCE` | NucleicAcidSourceEnum<br/>`DNA`, `RNA`, `Unknown` | Yes | Type of nucleic acid used for sequencing |
| `READ_INDICATOR` | ReadIndicatorEnum<br/>`Forward`, `Index`, `Reverse`, `Unknown` | Yes | Type of read (forward, reverse, index) |
| `REVERSE_TRANSCRIPTION_PRIMER` | ReverseTranscriptionPrimerEnum<br/>`Oligo-dT`, `Random Hexamer`, `Unknown` | Yes | Primer used for reverse transcription |
| `SINGLE_CELL_ISOLATION_METHOD` | See [SingleCellIsolationMethodEnum](#singlecellisolationmethod) enum below | Yes | Method used to isolate single cells |
| `SPIKE_IN` | SpikeInEnum<br/>`ERCC`, `None`, `Other`, `Unknown` | Yes | Type of spike-in used, if any |
...
| `SCRNASEQ_WORKFLOW_TYPE` | See [scRNAseqWorkflowTypeEnumLevel2](#scrnaseqworkflowtypelevel2) enum below | Yes | Generic name for the workflow used to analyze the dataset |
...
| `DATA_CATEGORY` | See [DataCategoryEnum](#datacategory) enum below | Yes | Specific content type of the data file |
...
| `MATRIX_TYPE` | MatrixTypeEnum<br/>`Batch Corrected Counts`, `Normalized Counts`, `Raw Counts`, `Scaled Counts` | Yes | Type of data stored in matrix |
...
| `SCRNASEQ_WORKFLOW_TYPE` | See [scRNAseqWorkflowTypeEnumLevel3_4](#scrnaseqworkflowtypelevel3-4) enum below | Yes | Generic name for the workflow used to analyze a data set |
