...
| Attribute | Type | Required | Description |
|-----------|------|----------|-------------|
| `ANNDATA_SCHEMA_VERSION` | string, pattern: `^0\.1$` | Yes | Version of AnnData schema (must be 0.1 for CellxGene compliance) |
...
| `FILE_FORMAT` | string, pattern: `^h5ad$` | Yes | Format of the file (only h5ad files accepted for Level 3/4) |
