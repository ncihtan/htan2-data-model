...
| `LIBRARY_SELECTION_METHOD` | LibrarySelectionMethodEnum<br/>`Hybrid Selection`, `PCR`, `RANDOM`, `other` | Yes | Method used for library selection |
...
| `MSI_STATUS` | MSIStatusEnum<br/>`MSI-H`, `MSI-L`, `MSS`, `Unknown` | No | MSI status |
| `MSI_WORKFLOW_LINK` | string | No | Link to MSI workflow |
| `SOMATIC_VARIANTS_SAMPLE_TYPE` | SomaticVariantsSampleTypeEnum<br/>`Metastatic`, `Normal`, `Other`, `Primary`, `Recurrent` | No | Type of sample for somatic variants |
