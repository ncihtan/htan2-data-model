...
def find_identifiers():
                    ...
                    identifiers.append(
                        {
                            ...
                            or "HTAN Participant ID (Primary Key)",
    ...
    biospecimen_file = (
        base_dir / "modules" / "Biospecimen" / "domains" / "biospecimen.yaml"
    )
                    ...
                    identifiers.append(
                        {
                            ...
                            "pattern": attr.pattern,
                            "description": attr.description
                            or "HTAN Biospecimen ID (Primary Key)",
                            "required": attr.required,
                        }
                    )
    ...
    spatial_panel_file = (
        base_dir / "modules" / "SpatialOmics" / "domains" / "spatial_panel.yaml"
    )
...
def generate_identifier_doc():
    ...
    with open(output_path, "w") as f:
        ...
        for ident in identifiers:
            ...
            f.write(
                f"| `{ident['name']}` | {ident['module']} | <code>{pattern_escaped}</code> | {required} | {description} |\n"
            )
