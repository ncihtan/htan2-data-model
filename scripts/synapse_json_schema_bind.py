...
def register_json_schema(
    org, schema_type: str, schema_json: json, version: str, schema_org_name: str
) -> str:
    ...
    try:
        schema = org.create_json_schema(
            schema_json, schema_type, semantic_version=num_version
        )
        uri = schema.uri
...
def create_fileview_from_schema(
    syn, schema_json: dict, parent_id: str, schema_name: str
) -> str:
...
def create_wiki_with_fileview_id(
    syn, entity_id: str, fileview_id: str, schema_name: str
):
...
"""
    ...
    try:
        ...
        syn.store(
            synapseclient.Wiki(
                owner=entity_id, title=f"{schema_name} Data View", markdown=wiki_content
            )
        )
...
def get_register_bind_schema(
    ...
    includes_ar: bool,
    no_bind: bool,
    create_fileview: bool,
):
    """Access JSON from URL, register the JSON schema, and bind the schema to the target entity."""

    schema_json, component_adjusted, base_component, version = get_schema_from_url(
        url, path
    )
    print(f"Registering JSON schema {component_adjusted} {version}")

    uri = register_json_schema(
        org, component_adjusted, schema_json, version, schema_org_name
    )
    ...
    if no_bind is None:
        ...
        if create_fileview:
            fileview_id = create_fileview_from_schema(
                syn, schema_json, target, component_adjusted
            )
            if fileview_id:
                print(f"✅ Fileview created with ID: {fileview_id}")
                # Create wiki page with fileview ID
                create_wiki_with_fileview_id(
                    syn, target, fileview_id, component_adjusted
                )
            else:
...
def main():
    ...
    args = get_args()
    target, url, path, org_name, includes_ar, no_bind, create_fileview = (
        args.t,
        args.l,
        args.p,
        args.n,
    ...
    if no_bind is not None:
        print(
            f"Warning ❗❗❗ Schema will not be bound to the entity if one was provided."
        )
        ...
        service, org, schema_org_name = get_schema_organization(
            schema_service, org_name
        )
    else:
