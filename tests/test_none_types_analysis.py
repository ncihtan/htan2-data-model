            ...
            if domains_dir.exists():
                ...
                for yaml_file in yaml_files:
                    if "enum" in yaml_file.name.lower():
                        ...
                        if none_types:
                            results["none_types_by_module"][module_name][
                                yaml_file.name
                            ] = none_types
...
def test_none_types_analysis():
    ...
    assert (
        stats["total_permissible_values"] > 0
    ), "Should find at least one permissible value"
...
def test_none_types_consistency():
    ...
    modules_with_none_types = [
        module for module, enums in none_types_by_module.items() if enums
    ]
    ...
    assert (
        len(modules_with_none_types) > 0
    ), "Should have none types in at least one module"
